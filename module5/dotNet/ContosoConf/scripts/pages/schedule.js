...
function downloadSchedule() {
    //INCOMPLETE: REFACTORING USING AJAX; MODULE 5, EXERCISE 3
    $.ajax({
        method: 'GET',
...
//ORIGINAL CODE FROM EXERCISE 1 & 2
    ...
    request.onload = function(){        
        if (request.status == 200) {
            try {
                ...
                alert('Error occured: ' + exception.message);
            }
        } else {
    ...
    if(isStarred) {
        request.onload = function(){        
            ...
            if (request.status == 200) {
                try {
                    var response = JSON.parse(this.responseText);
                    console.log('Star count is: ' + response.starCount);                    
                    if(response.starCount > 50) {
                        alert('This session is very popular!  Be sure to arrive early to get a seat, or RISK STANDING!!');     
                    }
                } catch (exception) {
                    alert('Parsing error occured: ' + exception.message);
                }
            } else {
                alert('Save Star function failed. Please try again later');
            }
        };
    }

    request.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded');
    //request.send("starred=" + sessionId);
